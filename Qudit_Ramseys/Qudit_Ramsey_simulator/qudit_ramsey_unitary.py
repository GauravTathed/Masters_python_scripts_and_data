...
class QuditUnitary(Calibrations):
    ...
    def __init__(
            ...
            line_amplitude_60: float = 0.000225,  # Gauss
            line_amplitude_180: float = 0.00009,  # Gauss
            ...
            freq_miscalibration: flb voat = 124.94,  # Hz
            detuning: float = 0,  # Hz
    ...
    def generate_frequency_miscalibration(self) -> float:
        ...
        return np.random.normal(loc=0.0, scale=self.freq_miscalibration/np.sqrt(2))
    ...
    def build_unitary(self,
                      ...
                      time: float = 1,
                      phase: float = 0,
                      real_detuning: float = 0) -> np.ndarray:
        ...
        trans_pitime = self.transition_pitimes[tuple(table_index)]  # us
        trans_Omega = np.pi / (trans_pitime)  # rad/s
    ...
    def evolve_qudit_Ramsey(self,
        ...
        for idx, trans in enumerate(self.pulse_dict['U1_transitions']):
            ...
            if idx == 0:
                ...
                # is a non-line triggered pulse - so larger error
                trans_pitime *= (1 + 7 * self.pulse_time_miscalibration *
                                 (np.random.random()))
            else:
            ...
            #     real_detuning = (
            #         2 * np.pi * trans_sens *
            #         self.generate_line_noise_detuning((1e6/60)*np.random.random()))
            # else:
            real_detuning = (
                2 * np.pi * trans_sens *
            ...
            new_unit = self.build_unitary(transition=trans,
                                          time=time,
                                          phase=phase,
